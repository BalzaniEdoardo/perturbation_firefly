...
class lfp_class(object):
    def __init__(
        ...
        if not lfp_beta is None:
            self.lfp_beta, self.lfp_beta_power = self.extract_lfp_x_band(
                lfp_beta, "lfp_beta"
            )

        if not lfp_alpha is None:
            self.lfp_alpha, self.lfp_alpha_power = self.extract_lfp_x_band(
                lfp_alpha, "lfp_alpha"
            )

        if not lfp_theta is None:
            self.lfp_theta, self.lfp_theta_power = self.extract_lfp_x_band(
                lfp_theta, "lfp_theta"
            )
        ...
        for chan in range(self.n_channels):
            for tr in range(num_trials):
                if self.compute_phase:
                    lfp_band_all[chan, tr] = np.array(
                        np.angle(
                            lfp_band[0, chan]["trials"][0, tr][band_label].flatten()
                        ),
                        dtype=np.float32,
                    )
                    lfp_power_all[chan, tr] = np.array(
                        np.abs(lfp_band[0, chan]["trials"][0, tr][band_label].flatten())
                        ** 2,
                        dtype=np.float32,
                    )
                else:
                    lfp_band_all[chan, tr] = lfp_band[0, chan]["trials"][0, tr][
                        band_label
                    ].flatten()
                    lfp_power_all = None
    ...
    def extract_phase(self, filter, channel_id_x_unit, unit_brain_area):
        ...
        for chan_un in range(num_units):
            ...
            select = (self.channel_id == channel_id_x_unit[chan_un]) * (
                self.brain_area == ba_unit
            )
            assert select.sum() == 1
    ...
    def extract_phase_x_unit(self, phase, filter, channel_id_x_unit, unit_brain_area):
        ...
        for chan_un in range(num_units):
            ...
            for idx in range(num_trials):
                ...
                phase_unit[chan_un, idx] = phase[
                    use_idx, tr
                ]  # np.angle(hilbert(self.lfp[use_idx,tr]))

        return phase_unit

    def extract_phase_from_band(
        self, lfp_band, filter, channel_id_x_unit, unit_brain_area
    ):
    ...
    def cut_phase(
        self, phase, edges, t_start=None, t_stop=None, select=None, idx0=None, idx1=None
    ):
        if select is None:
        ...
        for unit in range(phase.shape[0]):
            cut_phase[unit, :] = self.cut_continuous(
                phase[unit, :],
                edges,
                t_start=t_start,
                t_stop=t_stop,
...
if __name__ == "__main__":
    ...
    dat = loadmat(
        "/Volumes/WD Edo/firefly_analysis/LFP_band/DATASET/MST/m53s127_new.mat"
    )
    lfp_beta = loadmat(
        "/Volumes/WD Edo/firefly_analysis/LFP_band/DATASET/MST/lfp_beta_m53s127.mat"
    )
    lfp_alpha = loadmat(
        "/Volumes/WD Edo/firefly_analysis/LFP_band/DATASET/MST/lfp_alpha_m53s127.mat"
    )
    lfp_theta = loadmat(
        "/Volumes/WD Edo/firefly_analysis/LFP_band/DATASET/MST/lfp_theta_m53s127.mat"
    )
    ...
    lfp = lfp_class(
        dat,
        lfps_key,
        lfp_beta=lfp_beta["lfp_beta"],
        lfp_alpha=lfp_alpha["lfp_alpha"],
        lfp_theta=None,
        is_phase=True,
    )
    filter = info.get_all(True)
    phase = lfp.extract_phase_from_band(
        lfp.lfp_beta, filter, spk.channel_id, spk.brain_area
    )
