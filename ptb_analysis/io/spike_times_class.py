...
class spike_counts(object):
        ...
        if len(shape) == 2:
            ...
            if shape[1] != 1 and shape[0] != 1:
                ...
                for k in range(length):
                    # print(k)
                    try:
                        unpacked[k, :] = units[key][k].reshape(
                            N,
                        )
                    except ValueError:
    ...
    def bin_spikes(
        self, edges, t_start=None, t_stop=None, select=None, cutFirstLastTP=True
    ):
        ...
        for unt in range(self.num_units):
            for idx in range(spks_tmp.shape[1]):
                ...
                self.binned_spikes[unt, idx], _ = np.histogram(
                    self.spike_times[unt, tr], bins=bins
                )
                if unt == 0:
        ...
        for unt in range(self.num_units):
            for idx in range(spks_tmp.shape[1]):
                ...
                if np.isscalar(t_stop):
                    ...
                    t1 = float(t_stop[tr])
                sele_times = (self.spike_times[unt, tr] > t0) & (
                    self.spike_times[unt, tr] < t1
                )
